        ...
        if len(Y) == 1 or len(set(Y)) == 1:
            ...
            if max_ig == 0:
                ...
                if self.depth == self.max_depth:
                    ...
                    self.right = None
                    self.prediction = [
                        np.round(Y[X[:,best_col] < self.split].mean()), # takes majority class after splitting data for left
                        np.round(Y[X[:,best_col] >= self.split].mean()), # takes majority class after splitting data for right
                    ]
                else:
